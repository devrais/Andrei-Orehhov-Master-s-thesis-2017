...
* $Version: $
*
* Copyright (c) Enar Reilent 2009, Priit J�rv 2010,2011,2013
*
...
gint wg_column_to_index_id(void *db, gint column, gint type,
  gint *matchrec, gint reclen);
__device__ gint wg_multi_column_to_index_id(void *db, gint *columns, gint col_count,
  gint type, gint *matchrec, gint reclen);
...
#endif
__device__ gint wg_search_ttree_rightmost(void *db, gint rootoffset,
  gint key, gint *result, struct wg_tnode *rb_node);
__device__ gint wg_search_ttree_leftmost(void *db, gint rootoffset,
  gint key, gint *result, struct wg_tnode *lb_node);
__device__ gint wg_search_tnode_first(void *db, gint nodeoffset, gint key,
  gint column);
__device__ gint wg_search_tnode_last(void *db, gint nodeoffset, gint key,
  gint column);

gint wg_search_hash(void *db, gint index_id, gint *values, gint count);

#ifdef USE_INDEX_TEMPLATE
__device__ gint wg_match_template(void *db, wg_index_template *tmpl, void *rec);
#endif
