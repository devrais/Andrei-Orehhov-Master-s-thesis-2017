...
__device__  static gint init_db_subarea(void* db, void* area_header, gint index,
		gint size);
__device__  static gint alloc_db_segmentchunk(void* db, gint size); // allocates a next chunk from db memory segment
__device__  static gint init_syn_vars(void* db);
__device__  static gint init_extdb(void* db);
__device__  static gint init_db_index_area_header(void* db);
__device__  static gint init_logging(void* db);
__device__  static gint init_strhash_area(void* db, db_hash_area_header* areah);
__device__  static gint init_hash_subarea(void* db, db_hash_area_header* areah,
		gint arraylength);
__device__  static gint init_db_recptr_bitmap(void* db);
#ifdef USE_REASONER
__device__ static gint init_anonconst_table(void* db);
__device__ static gint intern_anonconst(void* db, char* str, gint enr);
#endif

__device__  static gint make_subarea_freelist(void* db, void* area_header,
		gint arrayindex);
__device__  static gint init_area_buckets(void* db, void* area_header);
__device__  static gint init_subarea_freespace(void* db, void* area_header,
		gint arrayindex);

__device__  static gint show_dballoc_error_nr(void* db, char* errmsg, gint nr);
__device__  static gint show_dballoc_error(void* db, char* errmsg);

__device__ static gint split_free(void* db, void* area_header, gint nr, gint* freebuckets, gint i);

__device__ static gint extend_fixedlen_area(void* db, void* area_header);
/* ====== Functions ============== */

__device__  static gint show_dballoc_error(void* db, char* errmsg) {
#ifdef WG_NO_ERRPRINT
#else
	printf("cuda db memory allocation error: %s\n", errmsg);
#endif
	return -1;
}
...
__device__ void wg_free_tnode(void* db, gint offset) {
  dbstore(db,offset,(dbmemsegh(db)->tnode_area_header).freelist);
  (dbmemsegh(db)->tnode_area_header).freelist=offset;
}

__device__ static gint extend_varlen_area(void* db, void* area_header, gint minbytes) {
  gint i;
  gint tmp;
...
__device__ void wg_free_word(void* db, gint offset) {
  dbstore(db,offset,(dbmemsegh(db)->word_area_header).freelist);
  (dbmemsegh(db)->word_area_header).freelist=offset;
}

__device__ gint wg_alloc_gints(void* db, void* area_header, gint nr) {
...
__device__ gint wg_alloc_fixlen_object(void* db, void* area_header) {
  db_area_header* areah;
  gint freelist;
...
__device__ gint init_area_buckets(void* db, void* area_header) {
	db_area_header* areah;
	gint* freebuckets;
	gint i;
...
__device__ gint wg_freebuckets_index(void* db, gint size) {
	gint i;
	gint cursize;

	if (size < EXACTBUCKETS_NR)
		return size;
	cursize = EXACTBUCKETS_NR * 2;
	for (i = 0; i < VARBUCKETS_NR; i++) {
		if (size < cursize)
			return EXACTBUCKETS_NR + i;
		cursize = cursize * 2;
	}
	return -1; // too large size, not enough buckets
}

__device__ void wg_free_listcell(void* db, gint offset) {
  dbstore(db,offset,(dbmemsegh(db)->listcell_area_header).freelist);
  (dbmemsegh(db)->listcell_area_header).freelist=offset;
}

__device__ gint init_subarea_freespace(void* db, void* area_header,
		gint arrayindex) {
	db_area_header* areah;
	gint* freebuckets;
	gint size;
	...
	if (arrayindex > 0) {
		...
		if (dv != 0 && dvsize >= MIN_VARLENOBJ_SIZE) {
			...
			freelist = freebuckets[dvindex];
			if (freelist != 0)
				dbstore(db, freelist + 2 * sizeof(gint), dv); // update prev ptr
			dbstore(db, dv + sizeof(gint), freelist); // store previous freelist
			dbstore(db, dv + 2 * sizeof(gint),
					dbaddr(db,&freebuckets[dvindex])); // store ptr to previous
			freebuckets[dvindex] = dv; // store offset to correct bucket
...
__device__  static gint init_strhash_area(void* db, db_hash_area_header* areah) {
	...
	if (STRHASH_SIZE > 0.01 && STRHASH_SIZE < 50) {
		arraylength = (gint) ((dbh->size + 1) * (STRHASH_SIZE / 100.0))
				/ sizeof(gint);
	} else {
...
__device__ gint wg_init_db_memsegment(void* db, gint key, gint size) {
	db_memsegment_header* dbh = dbmemsegh(db);
	gint tmp;
	gint free;
	gint i;
	...
	//datarec
	tmp = init_db_subarea(db, &(dbh->datarec_area_header), 0,
			INITIAL_SUBAREA_SIZE);
	if (tmp) {
	...
	//longstr
	tmp = init_db_subarea(db, &(dbh->longstr_area_header), 0,
			INITIAL_SUBAREA_SIZE);
	if (tmp) {
	...
	//listcell
	tmp = init_db_subarea(db, &(dbh->listcell_area_header), 0,
			INITIAL_SUBAREA_SIZE);
	if (tmp) {
	...
	//shortstr
	tmp = init_db_subarea(db, &(dbh->shortstr_area_header), 0,
			INITIAL_SUBAREA_SIZE);
	if (tmp) {
	...
	//word
	tmp = init_db_subarea(db, &(dbh->word_area_header), 0,
			INITIAL_SUBAREA_SIZE);
	if (tmp) {
	...
	//doubleword
	tmp = init_db_subarea(db, &(dbh->doubleword_area_header), 0,
			INITIAL_SUBAREA_SIZE);
	if (tmp) {
	 ...
	 */
	tmp = init_db_subarea(db, &(dbh->tnode_area_header), 0,
			INITIAL_SUBAREA_SIZE);
	if (tmp) {
	...
	tmp = init_db_subarea(db, &(dbh->indexhdr_area_header), 0,
			MINIMAL_SUBAREA_SIZE);
	if (tmp) {
	...
	tmp = init_db_subarea(db, &(dbh->indextmpl_area_header), 0,
			MINIMAL_SUBAREA_SIZE);
	if (tmp) {
		show_dballoc_error(db, " cannot create index header area");
		return -1;
	...
	tmp = init_db_subarea(db, &(dbh->indexhash_area_header), 0,
			INITIAL_SUBAREA_SIZE);
	if (tmp) {
...
__device__  static gint alloc_db_segmentchunk(void* db, gint size) {
	db_memsegment_header* dbh = dbmemsegh(db);
	gint lastfree;
	gint nextfree;
	gint i;
	...
	if (nextfree < 0) {
		show_dballoc_error_nr(db,
				"trying to allocate next segment exceeds positive int limit",
				size);
		return 0;
	}
	...
	if (nextfree >= (dbh->size)) {
		...
		show_dballoc_error_nr(db,
				"segment does not have enough space for the required chunk of size",
				size);
#endif
		return 0;
	}
...
__device__  static gint init_hash_subarea(void* db, db_hash_area_header* areah,
	...
	for (j = 0; j < arraylength; j++)
		dbstore(db, i + (j * sizeof(gint)), 0);
	//show_strhash(db);
	return 0;
}

__device__ gint wg_free_object(void* db, void* area_header, gint object) {
  gint size;
  gint i;
  gint* freebuckets;
...
__device__ void wg_free_shortstr(void* db, gint offset) {
...
__device__ void wg_free_doubleword(void* db, gint offset) {
